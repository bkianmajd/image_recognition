...
public:
  ...
  bool RegisterTemplate(TemplateId template_id,const std::string& image_name) override {
    // Find the image in the data file
    std::string image_with_path = data_file_path_ + image_name;
    // Find the image in the data file
    if(!CheckImage(image_with_path)) {
    ...
    template_map_.insert(
          std::pair<TemplateId,cv::Mat>(template_id, cv::imread(image_with_path,1)));
