...
namespace {
  ...
  const std::string kBigImg = "left01.jpg";
  const std::string kTemplateImg = "test.jpg";
...
public:
  RecognitionTest() :
  simple_recognition_(testing_main::kWorkspaceFromTestingMain + kFilePath) {}

protected:
    SimpleRecognition simple_recognition_;
};

TEST_F(RecognitionTest, ConstructDestruct) {
}

TEST_F(RecognitionTest, FileCheckTest) {
  std::string file_path = testing_main::kWorkspaceFromTestingMain + kFilePath;
  std::string test_file = file_path + "test.jpg";

  std::ifstream test_open;
  test_open.open(test_file);
  EXPECT_TRUE(test_open.is_open());
  test_open.close();
  EXPECT_FALSE(test_open.is_open());
}

TEST_F(RecognitionTest, RegisterTest) {
...
TEST_F(RecognitionTest, ImageRegisterTest) {
  EXPECT_TRUE(simple_recognition_.RegisterImage(kTemplateImg));
  EXPECT_TRUE(simple_recognition_.RegisterImage(kBigImg));
  EXPECT_FALSE(simple_recognition_.RegisterImage("no_file"));
}

TEST_F(RecognitionTest, TemplateStorageTest) {
  ...
  cv::Mat templ = cv::imread(testing_main::kWorkspaceFromTestingMain +
                             kFilePath + kTemplateImg, 1);
  EXPECT_EQ(it->second.cols, templ.cols);
}


TEST_F(RecognitionTest, GetTemplateMatchTest) {
