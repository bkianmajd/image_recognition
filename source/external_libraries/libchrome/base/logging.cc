...
PathString GetDefaultLogFile() {
  ...
  if (last_backslash != PathString::npos) log_name.erase(last_backslash + 1);
...
bool InitializeLogFileHandle() {
  ...
  if ((g_logging_destination & LOG_TO_FILE) != 0) {
    ...
    if (g_log_file == INVALID_HANDLE_VALUE || g_log_file == nullptr) {
      ...
      if (len == 0 || len > base::size(system_buffer)) return false;
...
bool BaseInitLoggingImpl(const LoggingSettings& settings) {
  ...
  if ((g_logging_destination & LOG_TO_FILE) == 0) return true;
  ...
  if (!g_log_file_name) g_log_file_name = new PathString();
...
int GetVlogLevelHelper(const char* file, size_t N) {
  ...
  return vlog_info ? vlog_info->GetVlogLevel(base::StringPiece(file, N - 1))
                   : GetVlogVerbosity();
}
...
LogMessageHandlerFunction GetLogMessageHandler() { return log_message_handler; }
...
void LogMessage::Init(const char* file, int line) {
  ...
  stream_ << '[';
  if (g_log_prefix) stream_ << g_log_prefix << ':';
  if (g_log_process_id) stream_ << CurrentProcessId() << ':';
  if (g_log_thread_id) stream_ << base::PlatformThread::CurrentId() << ':';
  if (g_log_timestamp) {
#if defined(OS_WIN)
...
void RawLog(int level, const char* message) {
  ...
  if (level == LOG_FATAL) base::debug::BreakDebugger();
}
...
#if defined(OS_WIN)
bool IsLoggingToFileEnabled() { return g_logging_destination & LOG_TO_FILE; }

std::wstring GetLogFileFullPath() {
  if (g_log_file_name) return *g_log_file_name;
  return std::wstring();
}
#endif
